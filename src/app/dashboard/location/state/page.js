...
export default function page() {
  ...
  const handleRedirectStateFormPage = (rowData) => {
    router.push(`/dashboard/location/state/form?id=${rowData?._id}`);
  };

  // deleted row
  const handleDeletedRow = async () => {
    const response = await apiCall(`/state/${isSingleRowData?._id}`, "DELETE");
    if (!response.error) {
    ...
    <>
      <div className="p-4 bg-white rounded-lg border border-gray-200">
        <div className="flex items-center justify-between">
          <h1 className="text-[18px] font-semibold">State</h1>
          <Button onClick={() => router.push("/dashboard/location/state/form")}>Add State</Button>
        </div>

        <div className="mt-5">
          <CustomTable columns={columns} data={stateTableData?.data} />
        </div>
      </div>
