...
export default function page() {
  ...
  const columns = [
    ...
    },
    {
      ...
      accessorKey: "image",
      cell: (cellInfo) => (
        <img src={cellInfo?.row?.original?.image} alt="facility" className="w-16 h-16 object-cover rounded-md border border-gray-300 shadow-sm mx-auto" />
      ),
    },
    {
      header: "Status",
      ...
      cell: (cellInfo) => (
        <div className="space-x-2">
          <button
            ...
            onClick={() => {
              handleRedirectFacilityFormPage(cellInfo?.row?.original);
            }}
          >
            Edit
          </button>
  ...
  const fetchFacility = async () => {
    const response = await apiCall("/facility");
    if (!response.error) {
      setFacilityTableData(response);
    }
  };
  ...
  // redirect to the facility form page
  const handleRedirectFacilityFormPage = (rowData) => {
    router.push(`/dashboard/facility/form?id=${rowData?._id}`);
  };

  // deleted row
  const handleDeletedRow = async () => {
    const response = await apiCall(`/facility/${isSingleRowData?._id}`, "DELETE");
    if (!response.error) {
    ...
    <>
      <div className="p-4 bg-white rounded-lg border border-gray-200">
        <div className="flex items-center justify-between">
          <h1 className="text-[18px] font-semibold">Facility</h1>
          <Button onClick={() => router.push("/dashboard/facility/form")}>Add Facility</Button>
        </div>

        <div className="mt-5">
          <CustomTable columns={columns} data={facilityTableData?.data} />
        </div>
      </div>
