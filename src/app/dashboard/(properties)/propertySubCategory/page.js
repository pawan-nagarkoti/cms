...
export default function page() {
  ...
  const [propertySubCategoryTableData, setPropertySubCategoryTableData] = useState([]);
  ...
  const columns = [
    ...
    },
    {
      ...
      accessorKey: "name",
      cell: (cellInfo) => cellInfo?.row?.original?.subCategoryName,
    },
    {
      header: "Sub Category Name",
      accessorKey: "name",
      cell: (cellInfo) => cellInfo?.row?.original?.categoryName?.name,
    },
    {
      header: "Image",
      accessorKey: "image",
      ...
      cell: (cellInfo) => (
        <div className="space-x-2">
          <button
            ...
            onClick={() => {
              handleRedirectPropertySubCategoryFormPage(cellInfo?.row?.original);
            }}
          >
            Edit
          </button>
  ...
  // fetch countries
  const fetchPropertySubCategory = async () => {
    const response = await apiCall("/propertySubCategory");
    if (!response.error) {
      setPropertySubCategoryTableData(response);
    }
  };
  ...
  // redirect to the propertySubCategory form page
  const handleRedirectPropertySubCategoryFormPage = (rowData) => {
    router.push(`/dashboard/propertySubCategory/form?id=${rowData?._id}`);
  };

  // deleted row
  const handleDeletedRow = async () => {
    const response = await apiCall(`/propertySubCategory/${isSingleRowData?._id}`, "DELETE");
    if (!response.error) {
    ...
    <>
      <div className="p-4 bg-white rounded-lg border border-gray-200">
        <div className="flex items-center justify-between">
          <h1 className="text-[18px] font-semibold">Property Sub Category</h1>
          <Button onClick={() => router.push("/dashboard/propertySubCategory/form")}>Add Property Sub Category</Button>
        </div>

        <div className="mt-5">
          <CustomTable columns={columns} data={propertySubCategoryTableData?.data} />
        </div>
      </div>
